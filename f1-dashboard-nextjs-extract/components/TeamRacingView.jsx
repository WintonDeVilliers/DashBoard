...
// Racing Track Component for reuse
const RacingTrackView = ({ teams, circuit, title, color }) => (
  <Card className={styles.racingTrackCard}>
    ...
    <CardContent className={styles.trackContent}>
      <div className={styles.racingTrack} data-testid={`racing-track-${circuit.toLowerCase()}`}>
        {teams.map((team, index) => (
          <div key={team.id} className={styles.trackLane} data-testid={`track-lane-${circuit.toLowerCase()}-${index}`}>
            {/* Track Lane */}
...
export default function TeamRacingView({ teamData, companyMetrics }) {
  const { monacoTeams, kyalamiTeams, sortedTeams } = useMemo(() => {
    if (!teamData) return { monacoTeams: [], kyalamiTeams: [], sortedTeams: [] };
    
    const sorted = [...teamData].sort((a, b) => b.team_achievement_rate - a.team_achievement_rate);
    
    const monaco = sorted.filter(team => team.circuit && team.circuit.toLowerCase() === 'monaco')
                          .sort((a, b) => b.team_achievement_rate - a.team_achievement_rate);
    
    const kyalami = sorted.filter(team => team.circuit && team.circuit.toLowerCase() === 'kyalami')
                           .sort((a, b) => b.team_achievement_rate - a.team_achievement_rate);
    
    return {
      monacoTeams: monaco,
      kyalamiTeams: kyalami,
      sortedTeams: sorted
    };
  }, [teamData]);
      ...
      <div className={styles.container}>
        ...
        <div className={styles.hero}>
          ...
          <p className={styles.heroSubtitle}>
            Supervisor performance by circuit (Drivers)
          </p>
        </div>

        {/* Monaco Circuit Racing Track */}
        <div className={styles.circuitSection}>
          <RacingTrackView 
            teams={monacoTeams}
            circuit="Monaco"
            title="🏎️ Live Positions - Monaco Circuit"
            color="#FF6B35"
          />
        </div>

        {/* Kyalami Circuit Racing Track */}
        <div className={styles.circuitSection}>
          <RacingTrackView 
            teams={kyalamiTeams}
            circuit="Kyalami"
            title="🏎️ Live Positions - Kyalami Circuit"
            color="#4ECDC4"
          />
        </div>
        ...
        <Card className={styles.leaderboardCard}>
          <CardHeader className={styles.leaderboardHeader}>
            ...
            <p className={styles.standingsSubtitle}>All supervisors ranked by performance</p>
          </CardHeader>
          <CardContent>
                  ...
                  >
                    ...
                    <div className={styles.tableCell}>
                      <span 
                        ...
                        style={{ 
                          color: team.circuit && team.circuit.toLowerCase() === 'monaco' ? '#FF6B35' : '#4ECDC4'
                        }}
                        data-testid={`circuit-${team.id}`}
                      >
                        {team.circuit && team.circuit.toLowerCase() === 'monaco' ? 'Monaco' : 'Kyalami'}
                      </span>
                    </div>
                    <div className={styles.tableCell}>
